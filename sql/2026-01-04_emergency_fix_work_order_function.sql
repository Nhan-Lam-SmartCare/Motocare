-- EMERGENCY FIX: Sửa function để cho phép staff tạo phiếu không cần branch check chặt
-- Ngày: 2026-01-04
-- Vấn đề: Function check branch quá strict gây lỗi UNAUTHORIZED
...
DECLARE
  ...
  v_user_branch TEXT;
BEGIN
  -- Get user's branch from profile (RELAXED CHECK)
  SELECT branch_id INTO v_user_branch
  FROM public.profiles
  WHERE id = auth.uid();
  
  -- ✅ FIX: Nếu không tìm thấy branch trong profile, dùng branch từ parameter
  IF v_user_branch IS NULL THEN
    v_user_branch := p_branch_id;
  END IF;
  
  -- ✅ FIX: Chỉ check nếu branch khác hẳn (cho phép NULL)
...
GRANT EXECUTE ON FUNCTION public.work_order_create_atomic TO anon;
GRANT EXECUTE ON FUNCTION public.work_order_create_atomic TO service_role;

COMMENT ON FUNCTION public.work_order_create_atomic IS 'Tạo phiếu sửa chữa - RELAXED BRANCH CHECK để cho phép staff tạo phiếu';

-- Kiểm tra function đã update
SELECT 

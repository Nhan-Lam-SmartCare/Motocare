-- Drop ALL versions of sale_create_atomic function
-- Date: 2025-11-17

-- Drop without specifying parameters (drops all overloads)
...
BEGIN
  ...
  -- Inventory deduction with row-level lock
  v_index := 0;
  WHILE v_index < v_items_count LOOP
    v_item := p_items->v_index;
    v_part_id := v_item->>'partId';
    ...
    -- Get current stock for this branch from JSONB with row lock
    ...
    -- Insert inventory transaction with camelCase columns
    INSERT INTO inventory_transactions(
      id, 
      type, 
      "partId",      -- Use quoted identifiers for camelCase
      "partName", 
      quantity, 
      date, 
      "unitPrice", 
  ...
  -- Check if any items had insufficient stock
  IF jsonb_array_length(v_insufficient) > 0 THEN
    RAISE EXCEPTION 'INSUFFICIENT_STOCK:%', v_insufficient::text;
  END IF;

  -- Insert sale record (sale_code will be auto-generated by trigger)
  INSERT INTO sales(
    id, 
    date, 
    items, 
...
COMMENT ON FUNCTION public.sale_create_atomic IS 'Tạo hóa đơn và cập nhật tồn kho + giao dịch tiền mặt trong 1 transaction (atomic). Auto-generates sale_code. Accepts UUID for user_id. Uses quoted identifiers for camelCase columns.';

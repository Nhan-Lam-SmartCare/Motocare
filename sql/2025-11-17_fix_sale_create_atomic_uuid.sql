-- Fix sale_create_atomic to accept UUID for p_user_id
-- Date: 2025-11-17
-- This updates the function signature to match the new userid column type
...
-- Create the updated function with UUID user_id
CREATE OR REPLACE FUNCTION public.sale_create_atomic(
  ...
  p_user_id UUID,  -- Changed from TEXT to UUID to match sales.userid column
  p_user_name TEXT,
  p_branch_id TEXT
) RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  WHILE v_index < v_items_count LOOP
    ...
    -- Get current stock for this branch from JSONB
    ...
    ELSE
      -- Update stock JSONB for this branch
      UPDATE parts
...
COMMENT ON FUNCTION public.sale_create_atomic IS 'Tạo hóa đơn và cập nhật tồn kho + giao dịch tiền mặt trong 1 transaction (atomic). Auto-generates sale_code. Accepts UUID for user_id.';

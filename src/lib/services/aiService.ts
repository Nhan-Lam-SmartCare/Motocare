...
function fallbackModelsFor(modelName: string): string[] {
  ...
  if (modelName.startsWith("gemini-3.1-pro")) {
    return ["gemini-3.5-flash", "gemini-3.1-flash-lite", "gemini-2.5-pro", "gemini-2.5-flash"];
  }

  if (modelName.startsWith("gemini-3.1-flash-lite")) {
  ...
  if (modelName.startsWith("deepseek-reasoner")) {
    return ["deepseek-chat", "gemini-3.5-flash", "gpt-4o-mini"];
  }

  if (modelName.startsWith("deepseek")) {
    return ["gemini-3.5-flash", "gpt-4o-mini"];
  }
...
export async function generateAiResponse(
  ...
  // 1. KMS Q&A — uses local RAG. If API key is available, use LLM to synthesize answer.
  if (feature === "kms_qa") {
    ...
    if (!apiKey) {
      const kmsResult = matchedContent
        ? `🤖 [Trợ lý SmartCare AI — Dựa trên tài liệu nội bộ (Mock RAG)]:\n\n${matchedContent.slice(0, 500)}...\n\n👉 Quy trình đã được phê duyệt tại chi nhánh ${branchId}.`
        : `🤖 [Trợ lý SmartCare AI (Mock RAG)]:\nKhông tìm thấy tài liệu khớp. Vui lòng liên hệ Quản lý kỹ thuật hoặc tạo SOP mới.`;

      await createAiLog({ feature, modelUsed: modelName + " [mock]", promptTokens: 350, completionTokens: 300, costUsd: 0, latencyMs: Date.now() - startTime, status: "success", branchId });
      return kmsResult;
    } else {
      // API Key is available, inject context and flow down to LLM generation
      variables.context = matchedContent || "Không tìm thấy tài liệu SOP nội bộ nào khớp với từ khóa.";
    }
  }
...
function buildDefaultUserPrompt(feature: string, v: Record<string, any>): string {
  switch (feature) {
    ...
    case "kms_qa":     return `Tài liệu liên quan:\n${v.context || ""}\n\nCâu hỏi người dùng: ${v.question || ""}`;
    ...
    case "script":     return `Bạn là nhà viết kịch bản video ngắn (TikTok/Reels/Shorts) chuyên nghiệp cho trung tâm sửa chữa và bảo dưỡng xe máy MotoCare (Nhạn Lâm SmartCare).
Hãy viết một kịch bản video cực kỳ cuốn hút, đánh đúng tâm lý khách hàng cho ý tưởng sau đây: "${v.ideaTitle || "Bảo dưỡng xe ga"}".
Thời lượng dự kiến của video: ${v.duration || 45} giây.

ĐỊNH HƯỚNG NỘI DUNG (CHUYÊN BIỆT LĨNH VỰC SỬA XE MÁY):
- Chủ đề tập trung vào các dịch vụ thực tế của tiệm: sửa chữa xe tay ga, xe số, vệ sinh nồi xe ga chống rung đầu, vệ sinh kim phun họng ga, làm sạch buồng đốt carbon, thay nhớt máy chính hãng, bảo dưỡng định kỳ xe Honda, Yamaha (Vision, Lead, SH, AirBlade, Wave, Exciter, Winner...).
- Văn phong: Chân thực, gần gũi như một người thợ sửa xe lành nghề chia sẻ kiến thức, cảnh báo hỏng hóc thực tế (ví dụ: lọc gió quá bẩn gây tốn xăng, dây curoa nứt dễ đứt giữa đường, bi nồi móp gây giật cục...).
- Giải pháp: Khuyên khách hàng bảo dưỡng định kỳ tại MotoCare để xe chạy êm, bốc, tiết kiệm xăng và an toàn tuyệt đối.

YÊU CẦU NỘI DUNG KỊCH BẢN:
1. Phân chia kịch bản thành các cấu trúc rõ ràng khoa học:
   - hook: Câu mở đầu kích thích tò mò hoặc cảnh báo nguy hiểm trong 3-5 giây đầu (ví dụ: "Đừng cố đi xe ga nếu thấy đầu xe rung bần bật thế này...", "Sự thật về lọc gió 2 năm chưa thay...").
   - problem: Nêu vấn đề hoặc triệu chứng lỗi xe thực tế người dùng hay gặp phải (ví dụ: giật cục, lì máy, hao xăng).
   - story: Tình huống thực tế thợ xe MotoCare tiếp nhận xe của khách hoặc dẫn dắt vấn đề.
   - solution: Quy trình xử lý kỹ thuật, bảo dưỡng chi tiết (ví dụ: vệ sinh côn nồi, thay nhớt chính hãng, bảo dưỡng...) và lời khuyên của thợ.
   - cta: Lời kêu gọi hành động ghé tiệm MotoCare để kiểm tra xe miễn phí hoặc đặt lịch bảo dưỡng định kỳ.
   - callToComment: Câu hỏi kích thích người xem tương tác (ví dụ: "Xe anh em bao lâu chưa thay nhớt? Comment bên dưới nhé!").
   - estimatedDuration: Số giây thời lượng ước tính thực tế (số nguyên, ví dụ: 45).

2. Toàn bộ ngôn ngữ phải viết bằng Tiếng Việt sinh động, hấp dẫn, dễ hiểu đối với khách hàng đại chúng. Tránh ngôn từ quá khô khan kỹ thuật nhưng phải đảm bảo tính chuyên nghiệp của thợ xe.

YÊU CẦU ĐỊNH DẠNG:
- Bạn chỉ được phép trả về một chuỗi JSON duy nhất, khớp chính xác định dạng bên dưới.
- TUYỆT ĐỐI KHÔNG viết thêm bất kỳ lời chào, lời mở đầu (ví dụ "Chào bạn, đây là kịch bản..."), văn bản thừa, hay lời kết nào ngoài đối tượng JSON này.

{
  "hook": "Câu mở đầu cuốn hút, cảnh báo lỗi xe cụ thể",
  "problem": "Mô tả triệu chứng xe yếu/lì/hao xăng/hỏng hóc",
  "story": "Câu chuyện thợ kỹ thuật tiếp nhận xe tại tiệm MotoCare",
  "solution": "Quy trình xử lý kỹ thuật, bảo dưỡng chi tiết và lời khuyên thợ xe",
  "cta": "Kêu gọi ghé MotoCare bảo dưỡng định kỳ và kiểm tra xe miễn phí",
  "callToComment": "Câu hỏi gợi mở tương tác bình luận về tình trạng xe",
  "estimatedDuration": ${v.duration || 45}
}`;
...
Và đây là danh sách các phụ tùng và dịch vụ bán chạy nhất trong 30 ngày qua tại chi nhánh, kèm theo đơn giá bán lẻ (price), giá vốn nhập (cost) và biên lợi nhuận (%) của chúng:
${soldItemsStr}
...
YÊU CẦU TỐI ƯU HÓA LỢI NHUẬN:
1. Hãy ưu tiên kết hợp các Dịch vụ công việc (có isService = true, vì đây là những hạng mục công thợ có biên lợi nhuận ròng gần 100%) và các Phụ tùng có biên lợi nhuận cao (> 40%) để tối đa hóa lợi nhuận ròng của gói combo.
2. Kết hợp thời tiết mùa mưa/ngập lụt (cần bảo dưỡng phanh, lốp bám đường, lọc gió, dầu láp chống nước) hoặc mùa khô/nắng nóng bụi bẩn (cần làm mát động cơ, vệ sinh kim phun buồng đốt) để đề xuất combo thiết thực nhất.
3. Các combo nên thiết kế nhắm thẳng vào các dòng xe phổ biến nhất của tiệm (Ví dụ: Honda Vision, Lead, Yamaha Exciter...) để tăng tỷ lệ chốt đơn của khách hàng đi dòng xe đó.
...
Dưới đây là danh sách các khách hàng đã lâu chưa ghé tiệm bảo dưỡng (quá 90 ngày) cần liên hệ nhắc lịch, kèm phân khúc chi tiêu (segment như VIP, Thân thiết, Có nguy cơ rời bỏ) và chi tiết lịch sử dịch vụ/linh kiện đã sử dụng lần cuối (lastService) của họ:
${candidatesStr}
...
YÊU CẦU CÁ NHÂN HÓA DỰA TRÊN LỰA CHỌN PHÂN KHÚC VÀ LỊCH SỬ DỊCH VỤ:
1. Đối với khách hàng phân khúc "VIP" hoặc "Thân thiết" (Loyal/VIP): Tin nhắn tri ân lịch sự, thân thiện, nhắc về việc xe đã đến hạn bảo dưỡng định kỳ sau khi đã làm các dịch vụ ở mục 'lastService'. Tặng thêm dịch vụ rửa xe miễn phí hoặc kiểm tra xe 10 hạng mục miễn phí.
...
3. Tham chiếu chính xác các bộ phận đã sửa/thay lần trước (trong 'lastService.parts' hoặc 'lastService.services') nếu có, để cá nhân hóa lời khuyên (ví dụ: 'Lần trước anh đã vệ sinh bộ nồi Vision, lần này đi được 3,000km nữa anh nên ghé tiệm thay nhớt máy định kỳ nhé...').
4. Sử dụng tên riêng, dòng xe họ đang đi, và ngày ghé thăm cuối để nội dung tin nhắn tự nhiên nhất có thể.
 
YÊU CẦU ĐẶC BIỆT:
NẾU danh sách khách hàng bị trống, bạn HÃY TỰ ĐỘNG GIẢ LẬP thông tin của 2 khách hàng mẫu (ví dụ: anh Nguyễn Văn Hùng đi Honda SH 150i bảo dưỡng lần cuối 3 tháng trước phân khúc VIP và thay bi nồi, chị Trần Thị Mai đi Honda Vision bảo dưỡng lần cuối 5 tháng trước phân khúc Có nguy cơ rời bỏ và thay dầu nhớt) và các bản soạn tin nhắn Zalo/SMS cá nhân hóa lịch sự để phục vụ hiển thị. TUYỆT ĐỐI không trả về mảng rỗng.

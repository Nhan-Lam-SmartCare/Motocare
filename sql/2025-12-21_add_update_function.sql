-- Add missing work_order_update_atomic function
-- This function handles updating work orders and adjusting reserved stock
...
BEGIN
  ...
  FOR v_index IN 0..(v_parts_count - 1) LOOP
    ...
    IF v_quantity_diff > 0 THEN
      SELECT 
        ...
        COALESCE((reserved->>v_branch_id)::int, 0)  -- ✅ FIXED: Use 'reserved' not 'reservedstock'
  ...
  -- Build return JSON with workOrder object (matching repository expectations)
...
COMMENT ON FUNCTION public.work_order_update_atomic IS 'Cập nhật phiếu sửa chữa - FIXED: Sử dụng column reserved thay vì reservedstock';

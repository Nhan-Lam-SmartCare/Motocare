-- Fix RPC functions to use correct column name 'reserved' instead of 'reservedstock'
-- Date: 2025-12-21
-- Issue: RPC functions reference 'reservedstock' but actual column is 'reserved'
...
-- Recreate with correct column name 'reserved'
...
BEGIN
    ...
    WHILE v_index < v_parts_count LOOP
      ...
      IF v_quantity > 0 THEN
        SELECT 
          ...
          COALESCE((reserved->>p_branch_id)::INT, 0)  -- ✅ FIXED: Use 'reserved' not 'reservedstock'
  ...
  -- Create payment transaction with ACTUAL amount (after discount)
...
COMMENT ON FUNCTION public.work_order_create_atomic IS 'Tạo phiếu sửa chữa - FIXED: Sử dụng column name đúng là reserved';

-- Success message
SELECT 'RPC function fixed to use correct column name: reserved' AS result;
